...
#---------------------------------------------------------------------------------
APP_TITLE        := trident
APP_DESCRIPTION  := Application for testing 3DS hardware
# APP_AUTHOR       :=
...
CFLAGS	:=	-g -Wall -O2 -fanalyzer -mword-relocations \
			-ffunction-sections \
			$(ARCH)
...
clean:
	@echo clean ...
	@rm -fr $(BUILD) $(TARGET).3dsx $(OUTPUT).smdh $(TARGET).elf $(GFXBUILD) *.map
